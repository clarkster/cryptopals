...
object Challenge33_ImplementDiffieHellman extends Challenge {
  ...
  override def main(args: Array[String]): Unit = {
    ...
    def genCheckKeys(diffieHellman: DiffieHellman) : Unit = {
      val keyPairA = diffieHellman.generate(System.currentTimeMillis().toInt)
      val keyPairB = diffieHellman.generate(System.currentTimeMillis().toInt * 2)

      val s = diffieHellman.session(keyPairA.priv, keyPairB.pub)
      val s2 = diffieHellman.session(keyPairB.priv, keyPairA.pub)
      println("s " + s)
      println("s2 " + s2)
      assert(s == s2)
    ...
    genCheckKeys(DiffieHellman(DiffieHellmanParams(BigInt(37), BigInt(5))))

    genCheckKeys(DiffieHellman())
  }
}
